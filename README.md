...
这是一个全栈 Web 应用，用于自动扫描 Excel 文件中的 IP 地址，并通过 [ip2location.io](https://ip2location.io) 接口获取详细的 IP 情报（如 ASN、地理位置、代理检测等），最后生成包含丰富数据的 Excel 文件供用户下载。

## 功能特性

- 🚀 **自动化处理**：自动识别 Excel 前 10 行中的 IP 列。
- 🛡️ **IP 风险检测**：集成 ip2location.io 接口，获取代理/VPN检测、地理位置、ISP 等信息。
...
- 获取 [ip2location.io](https://www.ip2location.io/) 的 API Key。
...
1. **设置环境变量**
   ...
   ```bash
   IP2LOCATION_KEY=your_actual_api_key_here
   ```
...
| `IP2LOCATION_KEY` | **[必填]** ip2location.io 的 API 密钥 | 无 |
